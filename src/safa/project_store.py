...
class ProjectStore:
    ...
    def store_project(self, version_id: str, project_data: ProjectData) -> None:
        self.project_data[ProjectStore.PROJECT_KEY][version_id] = project_data
        self.save()

    def get_project(self, version_id: str) -> ProjectData:
        self.has_project(version_id, assert_contains=True)
        return self.project_data[ProjectStore.PROJECT_KEY][version_id]

    def has_project(self, version_id: str, assert_contains: bool = False) -> bool:
        has_creds = version_id in self.project_data[ProjectStore.PROJECT_KEY]
    ...
    def store_credentials(self, email: str, token: str):
        self.project_data[ProjectStore.CREDENTIAL_KEY][email] = token
        self.save()

    def get_credentials(self, email: str) -> str:
        self.has_credentials(email, assert_contains=True)
        return self.project_data[ProjectStore.CREDENTIAL_KEY][email]

    def has_credentials(self, email: str, assert_contains: bool = False) -> bool:
        has_creds = email in self.project_data[ProjectStore.CREDENTIAL_KEY]
        if assert_contains and not has_creds:
    ...
    def __load_cache_file(self):
        ...
        cached_project_data = json_data.get(ProjectStore.PROJECT_KEY, {})
        cached_credential_data = json_data.get(ProjectStore.CREDENTIAL_KEY, {})
        self.project_data[ProjectStore.PROJECT_KEY].update(cached_project_data)
        self.project_data[ProjectStore.CREDENTIAL_KEY].update(cached_credential_data)

    @staticmethod
    def __create_empty_data() -> Dict:
        return {
            ProjectStore.CREDENTIAL_KEY: {},
            ProjectStore.PROJECT_KEY: {}
        }
